...
export default function ComputerSearch({ computers }) {
  ...
  const getPC = computers.map(({ id, computer_name }, counter) => {
    return [
      id,
      computer_name,
      counter
  ...
  const results = !searchTerm ? getPC : getPC.filter(pc =>
    pc[1].toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase())
  )
