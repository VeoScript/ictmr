...
export default function NoteSearch({ notes }) {
  ...
  const getNote = notes.map(({ image, title, slug }, counter) => {
    return [
      title,
      image,
      slug,
  ...
  const results = !searchTerm ? getNote : getNote.filter(note =>
    note[0].toLocaleLowerCase().includes(searchTerm.toLocaleLowerCase())
  )
              ...
              <div className="flex flex-row w-full h-auto max-h-[15rem] overflow-y-auto bg-opacity-75">
                ...
                >
                  <ul className="flex flex-col w-full">
                    {results.map(note => {
                      return (
                        <li className="flex flex-row items-center w-full border-b border-light-panther" key={note[3]}>
                          <Link href="/notes/[slug]" as={`/notes/${note[2]}`}>
                            <a className="flex flex-row items-center w-full px-3 py-2 transition ease-in-out duration-200 hover:bg-light-panther space-x-3">
                              <img className="w-12 h-12 rounded-full object-cover" src={note[1]} alt="avatar" />
                              <span className="font-normal text-lg text-scheme-pale">{note[0]}</span>
                            </a>
                          </Link>
                        </li>
                      )
